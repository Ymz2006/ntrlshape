...
SHAPES="Ashape3d 4shape3d Fshape3d Vshape3d Lshape3d rectangle"
MESH=datasets/3dshape/2denv4_zup.obj
LOGS=./.preplogs_2denv4
mkdir -p $LOGS

current () {  # shape device
    local shape=$1 dev=$2 ds=${1}_2denv4
    (
        set -e
...
worker () {  # index stride
    ...
    while [ $k -lt ${#work[@]} ]; do
        echo "[$w] start ${work[$k]}_2denv4 on $dev"
        current ${work[$k]} $dev
        rc=$?
        if [ $rc -eq 0 ]; then echo "[ok]   ${work[$k]}_2denv4"
        else echo "[FAIL] ${work[$k]}_2denv4 rc=$rc"; fi
        k=$((k+stride))
    done
}
...
ls -d datasets/3dshape/*_2denv4 testing_data/3dshape/*_2denv4 2>/dev/null

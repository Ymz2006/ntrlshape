"""OBJ based preprocessing for 3-D shape path planning -- DUAL (dist OR rot) band.

This is a variant of ``dataprocessing/preprocess_obj.py``.  It is identical in
every respect except for how the banded endpoint ``x0`` is selected: instead of
a single clearance band controlled by ``--margin`` / ``--offset``, there are two
...
Usage:
    python dataprocessing/preprocess_obj_band_both.py \
...
def main():
    ...
    mode = 'TESTING (independent collision-free pairs)' if args.testing_data \
        else 'TRAINING (dist-OR-rot band correlated pairs)'

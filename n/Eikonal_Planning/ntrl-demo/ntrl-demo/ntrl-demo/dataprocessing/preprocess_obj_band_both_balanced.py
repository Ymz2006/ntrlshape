"""OBJ preprocessing -- DUAL (dist OR rot) band with QUOTA-BALANCED x0.

This is a variant of ``dataprocessing/preprocess_obj_band_both.py``.  The band
definition, ``x1`` sampling, every recorded quantity and every output file are
unchanged; the **only** difference is that ``x0`` is no longer accepted purely by
the band test.  Uniform rejection sampling in the band leaves the
``(speed_dist, speed_angle)`` grid badly skewed -- most x0 pile into a handful of
cells while the tight corners stay starved -- so this script flattens that
occupancy:

  1. **Pilot.**  Sample ``--pilot_samples`` (default 10000) banded x0, x1 stage
     skipped, and bin them on a uniform ``--nbins_dist x --nbins_angle`` grid over
     ``speed_dist x speed_angle`` in [0,1] -- the same two columns written to
     ``speed_dists.npy`` / ``speed_angles.npy``.
  2. **Quota.**  Count the cells holding **more than** ``--min_cell_count``
     (default 10) pilot points; call that ``n_target``.  Every such cell gets the
     same quota ``num_samples // n_target``, and the integer remainder goes to the
     highest-speed targetable cell (largest speed_dist bin, then largest
     speed_angle bin), so the quotas sum to exactly ``--num_samples``.  Cells at
     or below the threshold are judged too rare to fill and get quota 0.
  3. **Balanced sampling.**  x0 is proposed exactly as before and must still pass
     the band test, but is additionally rejected once its cell's quota is spent.
     Quota is consumed when a full pair is accepted, never when x0 alone passes.

Note that ``num_samples`` is the quota pool, so ``--num_samples 10000`` gives the
literal ``10000 / n_target`` target per cell; larger totals scale it up.

Because rare cells are upsampled, the run is substantially slower per pair than
the unbalanced sampler -- that is the point, but it means a cell that barely
cleared ``min_cell_count`` can dominate the runtime.  A stall detector
(``--max_stall``) writes off cells that turn out to be unreachable and moves their
debt to the highest-speed cell so the total is still met.  ``--no_balance``
restores the original preprocess_obj_band_both.py behaviour exactly.

Everything below describes the band itself, which is unchanged.

The band: instead of a single clearance band controlled by
``--margin`` / ``--offset``, there are two
independent bands, one on translational clearance and one on rotational
clearance, and ``x0`` is kept if it falls in **either** of them:

    dist band :  offset_dist < clearance(x0)  < margin_dist
    rot  band :  offset_rot  < min_angle(x0)  < margin_rot   (radians)

    keep(x0)  =  collision-free  AND  clearance > offset_dist
                 AND  (in dist band  OR  in rot band)

``clearance > offset_dist`` stays a hard floor for both branches (as in the
original script) so no endpoint is ever recorded closer than the minimum-speed
clearance.  ``x1`` is unchanged: a random SE(3) displacement away from ``x0``,
kept iff collision-free with clearance ``> offset_dist``.

The rationale for the OR: a placement can be tightly constrained *rotationally*
(the shape is nearly wedged and cannot turn) while still having plenty of
translational clearance, and vice versa.  The single-band sampler only sees the
first kind of configuration; this one covers both.

The saved ``speed*.npy`` arrays keep exactly the formulas of the original script
(``speed_angles`` is still ``min_angle / pi``, i.e. the rot band bounds are used
for *sampling only*, not for renormalizing the angular speed) so that the
downstream trainers consume this dataset unchanged.
...
each training row is a correlated pair ``(x0, x1)`` where
    ...
    * ``x0`` is drawn in the **narrow band** described above (dist band OR rot
      band) -- only there does the eikonal / normal loss carry useful gradient
      signal;
    * ``x1`` is a random SE(3) displacement away from ``x0`` (uniform direction,
      ...
      it is collision-free with clearance ``> offset_dist``.

For each endpoint we record

    * ``speed``  -- ``clip(clearance / margin_dist, offset_dist/margin_dist, 1)``  (same formula
...
Outputs (written to ``--out``):
    ...
    trans_n.npy         (N, 12)  -- [trans_n0(6) | trans_n1(6)]  unit translation
                                    direction that reduces obstacle distance most
                                    (translation half non-zero, rotation half 0)
    rot_n.npy           (N, 12)  -- [rot_n0(6) | rot_n1(6)]  unit rotation axis that
                                    reduces rotational clearance most (rotation half
                                    non-zero, translation half 0)
    env.npy             (M, 3)   -- environment surface points
    meta.json                    -- shape_scale, margin_dist, offset_dist,
                                    margin_rot, offset_rot, ...

Usage:
    python dataprocessing/preprocess_obj_band_both_balanced.py \
        --env   datasets/3dshape/env1.obj \
        --shape datasets/3dshape/rectangle.obj \
        --out   datasets/3dshape/rectangle_env1 \
        --num_samples 400000 \
        --margin_dist 0.1  --offset_dist 0.01 \
        --margin_rot 0.5236  --offset_rot 0.001 \
        --shape_scale 1.0
"""
...
def calculate_dist(face_verts, env_points, centers):
    ...
    n_ws     : (B, 3)   unit workspace contact normal (direction the shape moves
                        to gain translational clearance).
    """
...
def evaluate_placements(configs, tet_verts_local, face_verts_local,
    ...
    min_angle : (B,) float, CPU    smallest shape-dir / env-dir angle (rotational clearance)
    normal  : (B, 6) float, CPU    unit SE(3) clearance normal (rotvec in /2pi units)
    trans_n : (B, 6) float, CPU    unit translation direction that *reduces* the
                                   distance to the obstacle the most. Only the
                                   first 3 dims are non-zero (= -n_ws); last 3 = 0.
    rot_n   : (B, 6) float, CPU    unit rotation axis that *reduces* the rotational
                                   clearance (min angle) the most. Only the last 3
                                   dims are non-zero (= unit(u_s x u_e)); first 3 = 0.
    ...
    # Map each binned slot back to its original env-point index. Needed to recover
    # the winning env point for the rotational-clearance direction (rot_n) below,
    # and for the optional min-angle visualization.
    ...
    # Track which (bin, shape-dir, env-point) realizes the running max cosine. This
    # winning pair defines the rotational-clearance direction (rot_n) and is also
    # used for the optional min-angle visualization.
    ...
    # ── Rotational-clearance direction (rot_n) ───────────────────────────────
    # The min angle is between the shape-surface direction u_s and the env
    # direction u_e (both from the placement centre). Rotating u_s towards u_e
    # reduces that angle the fastest about the axis (u_s x u_e); that unit axis,
    # placed in the rotation half of an SE(3) vector, is rot_n.
    ...
    u_s = shape_pt - t                                          # (B,3)
    u_s = u_s / (u_s.norm(dim=1, keepdim=True) + EPS)
    u_e = env_pt - t                                            # (B,3)
    u_e = u_e / (u_e.norm(dim=1, keepdim=True) + EPS)
    rot_axis = torch.linalg.cross(u_s, u_e, dim=1)               # (B,3)
    rot_axis = rot_axis / (rot_axis.norm(dim=1, keepdim=True) + EPS)
    zeros3 = torch.zeros(B, 3, device=device)
    rot_n = torch.cat([zeros3, rot_axis], dim=1)                 # (B,6)
    ...
    dist, normal, n_ws = calculate_dist(faces, env_kept, configs[:, 0:3])
    _prof_toc('clearance', device)

    # ── Translational-clearance direction (trans_n) ──────────────────────────
    # n_ws is the unit direction the shape moves to *gain* clearance, so moving
    # the opposite way (-n_ws) reduces the distance to the obstacle the fastest.
    # Placed in the translation half of an SE(3) vector, that is trans_n.
    trans_n = torch.cat([-n_ws, zeros3], dim=1)                  # (B,6)

    if return_angle_pts:
        return (is_free.cpu(), dist.cpu(), min_angle.cpu(), normal.cpu(),
                trans_n.cpu(), rot_n.cpu(), shape_pt.cpu(), env_pt.cpu())
    return (is_free.cpu(), dist.cpu(), min_angle.cpu(), normal.cpu(),
            trans_n.cpu(), rot_n.cpu())


# ---------------------------------------------------------------------------
# Correlated-pair rejection sampling -- mirrors preprocess_dxf.generate_valid_pairs
#
#   x0 : drawn in the narrow band  offset_dist < dist(x0)  < margin_dist
#                              OR  offset_rot  < angle(x0) < margin_rot
#        (with dist(x0) > offset_dist required in both cases)
#   x1 : x0 + random SE(3) displacement, kept iff collision-free and dist > offset_dist
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Quota-balanced x0 sampling
#
# Uniform rejection sampling in the band produces a badly skewed occupancy over
# the (speed_dist, speed_angle) grid -- most x0 pile into a few cells and the
# tight corners are starved.  These helpers run a cheap pilot, turn its
# occupancy into a flat per-cell quota, and let the main sampler accept x0 only
# while its cell still owes points.
# ---------------------------------------------------------------------------
def speeds_from_raw(dist, angle, margin_dist, offset_dist):
    """Raw clearance / min-angle -> the [0,1] speed proxies written to disk.

    Torch mirror of the ``speed_dists`` / ``speed_angles`` formulas in ``main``,
    so the binning axes here are exactly the columns of ``speed_dists.npy`` /
    ``speed_angles.npy``, not some separate internal quantity.
    """
    sd = torch.clamp(dist / margin_dist, min=offset_dist / margin_dist, max=1.0)
    sa = angle / np.pi
    return sd, sa


def cell_index(sd, sa, nbins_dist, nbins_angle):
    """(speed_dist, speed_angle) -> flat cell id on a uniform [0,1]^2 grid.

    Cell ``(ai, di)`` flattens to ``ai * nbins_dist + di``; the exact value 1.0
    lands in the last bin rather than falling off the end.
    """
    di = torch.clamp((sd * nbins_dist).long(), 0, nbins_dist - 1)
    ai = torch.clamp((sa * nbins_angle).long(), 0, nbins_angle - 1)
    return ai * nbins_dist + di


def sample_x0_pilot(number_x0, tet_verts_local, face_verts_local, env_points,
                    half_extent, margin_dist, offset_dist, margin_rot, offset_rot,
                    rad_points, rad_bins, batch_size=256, device='cuda', yrot=False):
    """Pilot pass: sample ``number_x0`` banded x0 and return their raw (dist, angle).

    The x0 half of ``generate_valid_pairs`` with the x1 stage removed -- same
    proposal distribution, same cheap gate, same dist-OR-rot band acceptance, so
    the occupancy it measures is exactly the one the unbalanced sampler produces.
    Skipping x1 makes it roughly twice as cheap per accepted point.
    """
    ...
    rad_max = float(rad_bins[-1])
    rot_band_reachable = margin_rot > np.pi

    dists = torch.zeros(number_x0)
    angles = torch.zeros(number_x0)
    count = 0
    while count < number_x0:
        ...
        feas0 = (m0 + R_shape >= offset_dist) & (
            (m0 - R_shape <= margin_dist) | rot_band_reachable | (m0 <= rad_max))
        if int(feas0.sum()) == 0:
            continue
        x0 = x0[feas0]

        free0, dist0, angle0, _n, _tn, _rn = evaluate_placements(
            x0, tet_verts_local, face_verts_local, env_t, rad_points, rad_bins, device)
        in_dist_band = (dist0 > offset_dist) & (dist0 < margin_dist)
        in_rot_band = (angle0 > offset_rot) & (angle0 < margin_rot)
        keep = free0 & (dist0 > offset_dist) & (in_dist_band | in_rot_band)

        nk = int(keep.sum())
        if nk == 0:
            continue
        take = min(nk, number_x0 - count)
        idx = torch.nonzero(keep, as_tuple=False).squeeze(1)[:take]
        dists[count:count + take] = dist0[idx]
        angles[count:count + take] = angle0[idx]
        count += take
        print('pilot: {} / {}'.format(count, number_x0))

    return dists, angles


def build_bin_targets(pilot_dist, pilot_angle, margin_dist, offset_dist,
                      nbins_dist, nbins_angle, quota_total, min_count=10):
    """Pilot occupancy -> a flat per-cell quota summing to exactly ``quota_total``.

    Cells holding **more than** ``min_count`` pilot points are "targetable"; the
    rest are treated as too rare to fill in reasonable time and get a quota of 0
    (x0 landing there is rejected).  Every targetable cell gets the same quota
    ``quota_total // n_target``, and the integer remainder is handed to the
    highest-speed targetable cell -- largest speed_dist bin, then largest
    speed_angle bin -- so the quotas still add up to ``quota_total``.

    Returns
    -------
    targets     : (nbins_angle * nbins_dist,) int64  per-cell quota, flat
    pilot_count : (nbins_angle * nbins_dist,) int64  pilot occupancy, flat
    extras_cell : int   flat id of the cell that absorbed the remainder
    """
    ncells = nbins_angle * nbins_dist
    sd, sa = speeds_from_raw(pilot_dist, pilot_angle, margin_dist, offset_dist)
    cells = cell_index(sd, sa, nbins_dist, nbins_angle)
    pilot_count = torch.bincount(cells, minlength=ncells)

    targetable = pilot_count > min_count
    n_target = int(targetable.sum())
    if n_target == 0:
        raise RuntimeError(
            'no grid cell holds more than {} pilot points -- lower --min_cell_count '
            'or raise --pilot_samples'.format(min_count))

    # Highest-speed targetable cell: max speed_dist bin first, then speed_angle.
    tgt_ids = torch.nonzero(targetable, as_tuple=False).squeeze(1)
    di = tgt_ids % nbins_dist
    ai = tgt_ids // nbins_dist
    extras_cell = int(tgt_ids[torch.argmax(di * nbins_angle + ai)])

    base = quota_total // n_target
    targets = torch.zeros(ncells, dtype=torch.long)
    targets[targetable] = base
    targets[extras_cell] += quota_total - base * n_target
    return targets, pilot_count, extras_cell


def print_cell_table(title, flat, nbins_dist, nbins_angle):
    """Print a flat per-cell array as a speed_angle x speed_dist table.

    Rows run high speed_angle -> low so the table reads like a plot, matching the
    grid printers in the analysis scripts.
    """
    grid = flat.reshape(nbins_angle, nbins_dist)
    print('\n' + title)
    hdr = ' angle\\dist '
    for d in range(nbins_dist):
        hdr += '%9s' % ('%.2f' % ((d + 0.5) / nbins_dist))
    print(hdr)
    for a in range(nbins_angle - 1, -1, -1):
        row = '%10s ' % ('%.2f' % ((a + 0.5) / nbins_angle))
        for d in range(nbins_dist):
            row += '%9d' % int(grid[a, d])
        print(row)
...
def generate_valid_pairs(number_pairs, tet_verts_local, face_verts_local,
                         ...
                         margin_dist, offset_dist, margin_rot, offset_rot,
                         rad_points, rad_bins, batch_size=256, device='cuda',
                         testing=False, yrot=False, track_angle_pts=False,
                         bin_targets=None, nbins_dist=8, nbins_angle=8,
                         extras_cell=0, max_stall=200):
    """Sample ``number_pairs`` SE(3) placement pairs.

    Training mode (default): correlated pairs -- ``x0`` is drawn in the narrow
    band, defined here as satisfying *either* the translational band
    ``offset_dist < clearance < margin_dist`` *or* the rotational band
    ``offset_rot < min_angle < margin_rot`` (with ``clearance > offset_dist``
    required either way), and ``x1`` is a random SE(3) displacement away from it,
    kept iff collision-free with clearance > offset_dist.

    Quota-balanced mode (``bin_targets`` given, training only): on top of the band
    test, an x0 is accepted only while its (speed_dist, speed_angle) cell still
    owes points, and a cell stops accepting the moment its quota is met.  The band
    test itself is untouched -- this only reshapes *which* of the banded x0 are
    kept, so the run finishes with exactly ``bin_targets`` points per cell.  The
    quota is spent when a full pair is accepted (x1 can still fail), never when
    x0 alone passes.
    ...
    trans_normals : (number_pairs, 12)  unit translation directions that reduce the
        distance to the obstacle the most [trans_n0(6) | trans_n1(6)] (only the
        translation half is non-zero).
    rot_normals : (number_pairs, 12)  unit rotation axes that reduce the rotational
        clearance the most [rot_n0(6) | rot_n1(6)] (only the rotation half is non-zero).
    ...
    trans_normals = torch.zeros(number_pairs, 12)
    rot_normals = torch.zeros(number_pairs, 12)
    ...
    R_shape = float(face_verts_local.reshape(-1, 3).norm(dim=1).max())
    # Largest radial shell of the shape-surface points.  evaluate_placements drops
    # every env point farther than this from the placement centre before the angle
    # reduction, so a placement whose *nearest* env point is beyond it has all
    # shells empty and therefore min_angle == pi exactly.  Such a placement can
    # never fall inside a rot band with margin_rot <= pi, which makes this a valid
    # (conservative) broad-phase gate for the rotational branch.
    rad_max = float(rad_bins[-1])
    rot_band_reachable = margin_rot > np.pi     # band open at the top -> gate is a no-op

    # Per-cell quota still outstanding.  None -> unbalanced (original) behaviour.
    balanced = bin_targets is not None and not testing
    remaining = bin_targets.clone() if balanced else None
    stall = 0                 # consecutive batches that accepted nothing
    redistributed = False     # the stall escape hatch fires at most once

    def _eval0(cfg):
        """Full x0 evaluation; returns
        (free, dist, angle, normal, trans_n, rot_n, shp, envp), with shp/envp =
        None unless ``track_angle_pts``."""
        if track_angle_pts:
            return evaluate_placements(
                cfg, tet_verts_local, face_verts_local, env_t, rad_points, rad_bins,
                device, return_angle_pts=True)
        f, d, a, n, tn, rn = evaluate_placements(
            cfg, tet_verts_local, face_verts_local, env_t, rad_points, rad_bins, device)
        return f, d, a, n, tn, rn, None, None
    ...
    while count < number_pairs:
        # ── Stall escape hatch ───────────────────────────────────────────────
        # A cell that cleared the pilot's min_count can still be far too rare to
        # fill at the requested quota, which would spin here forever.  After
        # max_stall fruitless batches, write those cells off and hand their debt
        # to the highest-speed cell so the run still hits `number_pairs`.  If that
        # does not unstick it either, give up and return short rather than hang.
        if balanced and stall > max_stall:
            hard = torch.nonzero(remaining > 0, as_tuple=False).squeeze(1)
            if not redistributed:
                moved = int(remaining[hard].sum())
                print('WARNING: {} batches accepted nothing; {} cell(s) still owe {} '
                      'points and look unreachable -- moving that quota to the '
                      'highest-speed cell {}.'.format(
                          max_stall, int(hard.numel()), moved, extras_cell))
                remaining[hard] = 0
                remaining[extras_cell] += moved
                redistributed = True
                stall = 0
            else:
                print('WARNING: still stalled after redistributing; giving up '
                      '{} points short.'.format(int(remaining.sum())))
                break
        # Counted up here so every `continue` path below feeds the stall detector;
        # a successful accept resets it to 0.
        stall += 1
        ...
        if testing:
            ...
            # #1 cheap gate: both endpoints require clearance > offset_dist, so drop
            # any pair where an endpoint is provably too close (m + R < offset_dist).
            m0 = _min_center_dist(x0[:, 0:3], env_t, device)
            m1 = _min_center_dist(x1[:, 0:3], env_t, device)
            feas = (m0 + R_shape >= offset_dist) & (m1 + R_shape >= offset_dist)
            if int(feas.sum()) == 0:
                continue
            x0, x1 = x0[feas], x1[feas]

            free0, dist0, angle0, normal0, trans0, rot0, shp0, envp0 = _eval0(x0)
            free1, dist1, angle1, normal1, trans1, rot1 = evaluate_placements(
                x1, tet_verts_local, face_verts_local, env_t, rad_points, rad_bins, device)
            keep = free0 & free1 & (dist0 > offset_dist) & (dist1 > offset_dist)
        else:
            # ── Training: x0 in narrow band, x1 a reachable displacement ──
            # #1 cheap gate on x0's band feasibility.  x0 qualifies through the
            # dist band or the rot band, so the gate is the OR of the two cheap
            # tests -- and both branches share the clearance > offset_dist floor
            # (clearance <= m + R, so m + R < offset_dist is provably out).
            #   dist branch: clearance >= m - R, so m - R > margin_dist is out.
            #   rot  branch: min_angle < pi requires an env point inside the
            #                shape's radial shells, i.e. m <= rad_max.
            m0 = _min_center_dist(x0[:, 0:3], env_t, device)
            feas0 = (m0 + R_shape >= offset_dist) & (
                (m0 - R_shape <= margin_dist) | rot_band_reachable | (m0 <= rad_max))
            if int(feas0.sum()) == 0:
            ...
            free0, dist0, angle0, normal0, trans0, rot0, shp0, envp0 = _eval0(x0)
            # Narrow band: in the translational band OR in the rotational band,
            # with the clearance floor enforced either way.
            in_dist_band = (dist0 > offset_dist) & (dist0 < margin_dist)
            in_rot_band = (angle0 > offset_rot) & (angle0 < margin_rot)
            keep_x0 = free0 & (dist0 > offset_dist) & (in_dist_band | in_rot_band)
            # #3 quota gate: drop banded x0 whose cell is already full.  Cheap and
            # early, so a saturated cell costs nothing beyond the x0 evaluation it
            # already paid for -- no x1 is ever built for it.
            sd0, sa0 = speeds_from_raw(dist0, angle0, margin_dist, offset_dist)
            cell0 = cell_index(sd0, sa0, nbins_dist, nbins_angle)
            if balanced:
                keep_x0 = keep_x0 & (remaining[cell0] > 0)
            if int(keep_x0.sum()) == 0:
                continue
            # #2 keep only x0 survivors, then build / evaluate x1 for *those* alone.
            x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0 = _idx(
                keep_x0, x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0)
            cell0 = cell0[keep_x0]
            ...
            if int(in_bbox.sum()) == 0:
                continue
            x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0 = _idx(
                in_bbox, x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0)
            x1 = x1[in_bbox]
            cell0 = cell0[in_bbox]

            # #1 cheap gate on x1 (only needs clearance > offset_dist -> drop too-close).
            feas1 = _min_center_dist(x1[:, 0:3], env_t, device) + R_shape >= offset_dist
            if int(feas1.sum()) == 0:
                continue
            x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0 = _idx(
                feas1, x0, dist0, angle0, normal0, trans0, rot0, shp0, envp0)
            x1 = x1[feas1]
            cell0 = cell0[feas1]

            free1, dist1, angle1, normal1, trans1, rot1 = evaluate_placements(
                x1, tet_verts_local, face_verts_local, env_t, rad_points, rad_bins, device)
            keep = free1 & (dist1 > offset_dist)                   # x0 already in band
        ...
        if balanced:
            # The x0 gate only checked that each cell had quota left; a single batch
            # can still carry more survivors for a cell than it owes.  Rank each
            # candidate within its own cell (order-preserving) and keep the first
            # `remaining` of them, so no cell can overshoot its target.
            c = cell0[idx]
            order = torch.argsort(c, stable=True)
            csorted = c[order]
            counts = torch.bincount(csorted, minlength=remaining.numel())
            starts = torch.cumsum(counts, 0) - counts
            rank_sorted = torch.arange(csorted.numel()) - starts[csorted]
            rank = torch.empty_like(rank_sorted)
            rank[order] = rank_sorted
            idx = idx[rank < remaining[c]]
        idx = idx[:number_pairs - count]
        take = int(idx.numel())
        if take == 0:
            continue
        if balanced:
            remaining -= torch.bincount(cell0[idx], minlength=remaining.numel())
        stall = 0
        ...
        trans_normals[count:count + take, 0:6] = trans0[idx]
        trans_normals[count:count + take, 6:12] = trans1[idx]
        rot_normals[count:count + take, 0:6] = rot0[idx]
        rot_normals[count:count + take, 6:12] = rot1[idx]
        if track_angle_pts:
            ang_shape[count:count + take] = shp0[idx]
            ang_env[count:count + take] = envp0[idx]
        count += take
        if balanced:
            print('generated: {} / {}   cells unfilled: {}'.format(
                count, number_pairs, int((remaining > 0).sum())))
        else:
            print('generated: {} / {}'.format(count, number_pairs))

    if balanced and count < number_pairs:
        # Only reachable via the stall escape hatch below giving up.
        print('WARNING: stopped {} pairs short of the target'.format(number_pairs - count))
        pairs, dists, angles = pairs[:count], dists[:count], angles[:count]
        normals, trans_normals = normals[:count], trans_normals[:count]
        rot_normals = rot_normals[:count]
        if track_angle_pts:
            ang_shape, ang_env = ang_shape[:count], ang_env[:count]

    return (pairs, dists, angles, normals, trans_normals, rot_normals,
            ang_shape, ang_env)
...
def main():
    ...
    parser.add_argument('--margin_dist', type=float, default=0.05,
                        help='Upper translational band: x0 is in the dist band if '
                             'its clearance < margin_dist; maps to speed=1.')
    parser.add_argument('--offset_dist', type=float, default=0.001,
                        help='Lower translational band: x0 must have clearance > '
                             'offset_dist (a hard floor, required even when x0 '
                             'qualifies through the rot band), and x1 (paired goal) '
                             'must also have clearance > offset_dist. Maps to the '
                             'minimum speed value offset_dist/margin_dist.')
    parser.add_argument('--margin_rot', type=float, default=np.pi / 6,
                        help='Upper rotational band, in radians: x0 is in the rot '
                             'band if its min-angle clearance < margin_rot.')
    parser.add_argument('--offset_rot', type=float, default=0.001,
                        help='Lower rotational band, in radians: x0 is in the rot '
                             'band only if its min-angle clearance > offset_rot.')
    parser.add_argument('--pilot_samples', type=int, default=10000,
                        help='Size of the preliminary x0-only pass whose occupancy '
                             'defines the per-cell quotas.')
    parser.add_argument('--nbins_dist', type=int, default=8,
                        help='speed_dist bins (uniform over [0,1]) for the quota grid.')
    parser.add_argument('--nbins_angle', type=int, default=8,
                        help='speed_angle bins (uniform over [0,1]) for the quota grid.')
    parser.add_argument('--min_cell_count', type=int, default=10,
                        help='A grid cell is targetable only if it holds MORE than '
                             'this many pilot points; rarer cells get quota 0.')
    parser.add_argument('--max_stall', type=int, default=200,
                        help='Consecutive batches accepting nothing before the '
                             'unreachable cells\' quota is moved to the highest-speed cell.')
    parser.add_argument('--no_balance', action='store_true',
                        help='Skip the pilot and the quotas: sample exactly like '
                             'preprocess_obj_band_both.py.')
    ...
    mode = 'TESTING (independent collision-free pairs)' if args.testing_data \
        else ('TRAINING (dist-OR-rot band correlated pairs, unbalanced)'
              if args.no_balance
              else 'TRAINING (dist-OR-rot band correlated pairs, quota-balanced x0)')
    print('Sampling {} (x0, x1) pairs [{}]   dist band=({}, {})  '
          'rot band=({}, {}) rad  ...'.format(
              num_pairs, mode, args.offset_dist, args.margin_dist,
              args.offset_rot, args.margin_rot))
    t0 = time.time()

    radius_points, radius_bins = generate_radius_surface_points(
        V_sh_local, F_sh, args.num_radius_points, args.radius_bins)

    # ── Pilot pass -> per-cell quotas ────────────────────────────────────────
    # Testing pairs are not banded, so there is nothing to balance there.
    bin_targets = extras_cell = None
    if not args.testing_data and not args.no_balance:
        print('\nPilot: sampling {} x0 to measure the speed_dist x speed_angle '
              'occupancy ...'.format(args.pilot_samples))
        t_pilot = time.time()
        pilot_dist, pilot_angle = sample_x0_pilot(
            args.pilot_samples, tet_verts_local, face_verts_local, env_points,
            half_extent, margin_dist=args.margin_dist, offset_dist=args.offset_dist,
            margin_rot=args.margin_rot, offset_rot=args.offset_rot,
            rad_points=radius_points, rad_bins=radius_bins,
            batch_size=args.batch_size, device=args.device, yrot=args.yrot)
        print('Pilot done in {:.1f}s'.format(time.time() - t_pilot))

        bin_targets, pilot_count, extras_cell = build_bin_targets(
            pilot_dist, pilot_angle, args.margin_dist, args.offset_dist,
            args.nbins_dist, args.nbins_angle, quota_total=num_pairs,
            min_count=args.min_cell_count)

        n_target = int((bin_targets > 0).sum())
        base = num_pairs // n_target
        print_cell_table('PILOT occupancy ({} points, {} cells):'.format(
            args.pilot_samples, args.nbins_angle * args.nbins_dist),
            pilot_count, args.nbins_dist, args.nbins_angle)
        print('\ncells with > {} pilot points : {} of {}'.format(
            args.min_cell_count, n_target, args.nbins_angle * args.nbins_dist))
        print('target per targetable cell   : {} // {} = {}  (+{} remainder -> '
              'cell {}, speed_dist bin {} / speed_angle bin {})'.format(
                  num_pairs, n_target, base, num_pairs - base * n_target,
                  extras_cell, extras_cell % args.nbins_dist,
                  extras_cell // args.nbins_dist))
        print_cell_table('TARGET per cell (sums to {}):'.format(int(bin_targets.sum())),
                         bin_targets, args.nbins_dist, args.nbins_angle)
        print('')

    pairs, dists, angles, normals, trans_normals, rot_normals, ang_shape, ang_env = generate_valid_pairs(
        num_pairs, tet_verts_local, face_verts_local, env_points, half_extent,
        margin_dist=args.margin_dist, offset_dist=args.offset_dist,
        margin_rot=args.margin_rot, offset_rot=args.offset_rot,
        ...
        bin_targets=bin_targets, nbins_dist=args.nbins_dist,
        nbins_angle=args.nbins_angle,
        extras_cell=extras_cell if extras_cell is not None else 0,
    ...
    trans_normals = trans_normals.cpu().numpy()   # (N, 12)  [trans_n0 | trans_n1]
    rot_normals = rot_normals.cpu().numpy()       # (N, 12)  [rot_n0 | rot_n1]

    # Eikonal speed term: clearance clipped into [offset_dist/margin_dist, 1].
    # Unchanged from preprocess_obj.py -- the rot band bounds affect *which* x0 are
    # sampled, not how the angular speed is normalized (the trainer applies its own
    # angle_max to speed_angles, which stays angle/pi).

    speed_pairs = (np.clip(dists / args.margin_dist,
                          a_min=args.offset_dist / args.margin_dist, a_max=1.0) + angles/np.pi)/2
    speed_angles = (angles/np.pi)
    speed_dists = np.clip(dists / args.margin_dist,
                          a_min=args.offset_dist / args.margin_dist, a_max=1.0)
    ...
    print('speed x0  : [{:.3f}, {:.3f}]   (band [{:.3f}, 1.0])'.format(
        ...
        args.offset_dist / args.margin_dist))
    print('speed x1  : [{:.3f}, {:.3f}]'.format(
        speed_pairs[:, 1].min(), speed_pairs[:, 1].max()))
    if not args.testing_data:
        # How each accepted x0 qualified -- the two bands overlap, so the counts sum
        # to more than N.
        in_d = (dists[:, 0] > args.offset_dist) & (dists[:, 0] < args.margin_dist)
        in_r = (angles[:, 0] > args.offset_rot) & (angles[:, 0] < args.margin_rot)
        print('x0 band   : dist {} ({:.1%})   rot {} ({:.1%})   both {} ({:.1%})'.format(
            int(in_d.sum()), in_d.mean(),
            int(in_r.sum()), in_r.mean(),
            int((in_d & in_r).sum()), (in_d & in_r).mean()))

    if bin_targets is not None:
        # Closes the loop: bin the x0 actually written out on the same grid and
        # check it against the quota it was sampled to hit.
        achieved = torch.bincount(
            cell_index(torch.tensor(speed_dists[:, 0]),
                       torch.tensor(speed_angles[:, 0]),
                       args.nbins_dist, args.nbins_angle),
            minlength=args.nbins_angle * args.nbins_dist)
        print_cell_table('ACHIEVED occupancy per cell:', achieved,
                         args.nbins_dist, args.nbins_angle)
        miss = (achieved - bin_targets).abs()
        print('\nmax |achieved - target| over cells: {}  (0 = every quota hit)'.format(
            int(miss.max())))
    ...
    np.save(os.path.join(args.out, 'trans_n'), trans_normals)
    np.save(os.path.join(args.out, 'rot_n'), rot_normals)
    ...
    meta = {
        ...
        'margin_dist': float(args.margin_dist),
        'offset_dist': float(args.offset_dist),
        'margin_rot':  float(args.margin_rot),
        'offset_rot':  float(args.offset_rot),
        'band_mode':   'dist_or_rot',
        'x0_balanced': bool(bin_targets is not None),
        'pilot_samples': int(args.pilot_samples),
        'nbins_dist':  int(args.nbins_dist),
        'nbins_angle': int(args.nbins_angle),
        'min_cell_count': int(args.min_cell_count),
        # Back-compat aliases: anything downstream that still reads meta['margin'] /
        # meta['offset'] gets the translational band, which is what those keys meant.
        'margin':      float(args.margin_dist),
        'offset':      float(args.offset_dist),
    ...
    print('  trans_n.npy       ', trans_normals.shape)
    print('  rot_n.npy         ', rot_normals.shape)

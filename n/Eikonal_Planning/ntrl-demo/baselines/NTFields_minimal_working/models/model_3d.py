...
class NN(torch.nn.Module):
    ...
    def __init__(self, device, dim, model_size):#10
        ...
        if model_size == 0 or model_size == 1:
            self.nl1=4 #5
            self.nl2=4 #7
        else:
            self.nl1=5
    ...
    def env_features_old(self, coords, f_0, f_1):
...
class Model():
    def __init__(self, ModelPath, DataPath, dim, pos,device='cpu', model_size=2):
        ...
        self.Params['Training']['Batch Size'] = 7500 #10000
        self.Params['Training']['Number of Epochs'] = 20000 #20000
        ...
        # 0 consumes the complete dataloader. A small positive value is useful
        # for smoke tests, but does not reproduce the bundled 2000-epoch model.
        self.Params['Training']['Max Batches Per Epoch'] = 6
        # Divisor applied to the raw configurations by models/database.py; 10.0
        # normalizes the bundled Aloha arrays, datasets already in [-0.5, 0.5]
        # (the 3-D shape SE(3) sets) need 1.0.
        self.Params['Training']['Data Scale'] = 10.0
        # An epoch is rejected and replayed from an earlier state when its mean
        # loss exceeds ``Repeat Ratio`` times the previous epoch's.  ``prev_diff``
        # starts at the constant 1.0, so on datasets whose initial loss sits
        # above the ratio (the 6-D shape sets start near 2.0) epoch 1 can never
        # be accepted; relax the ratio, and/or set ``Max Repeats Per Epoch`` to
        # accept an epoch after that many replays, to keep training moving.
        self.Params['Training']['Repeat Ratio'] = 1.2
        self.Params['Training']['Max Repeats Per Epoch'] = 0
    ...
    def train(self):
        ...
        self.network = NN(self.Params['Device'],self.dim, self.model_size)
        ...
        self.optimizer = torch.optim.AdamW(
            ...
            ,weight_decay=0.1)
        print("Model size: ", sum(p.numel() for p in self.network.parameters()))
        
        self.dataset = db.Database(self.Params['DataPath'], self.Params['Training']['Data Scale'])
        ...
        # WeightedRandomSampler + DataLoader gathers one row at a time in
        # Python, which dominates the runtime on the 800k-row shape datasets.
        # A single multinomial draw with replacement is the same distribution,
        # and slicing the resident tensor with it avoids the per-row gather and
        # the per-batch host-to-device copy.  Only the first
        # ``Max Batches Per Epoch`` batches are consumed, so draw exactly that
        # many; 0 (reproduction mode) keeps a full-length epoch.
        self.dataset.send_device(self.Params['Device'])
        weights = weights.to(torch.device(self.Params['Device']))

        def train_batches_wei():
            batch_size = int(self.Params['Training']['Batch Size'])
            n_batches = self.Params['Training']['Max Batches Per Epoch']
            if n_batches <= 0:
                n_batches = max(1, len(self.dataset) // batch_size)
            index = torch.multinomial(weights, n_batches * batch_size, replacement=True)
            for b in range(n_batches):
                batch = index[b * batch_size:(b + 1) * batch_size]
                yield self.dataset.data[batch], batch
        ...
        for epoch in range(1, self.Params['Training']['Number of Epochs']+1):
            ...
            while True:
                ...
                for i, data in enumerate(train_batches_wei(), 0):
                    max_batches = self.Params['Training']['Max Batches Per Epoch']
                    ...
                    loss_value, loss_n, wv = self.Loss(points, speed)
                ...
                total_train_loss /= batches_processed
                total_diff /= batches_processed

                current_diff = total_diff
                diff_ratio = current_diff/prev_diff
            
                ratio_limit = self.Params['Training']['Repeat Ratio']
                max_repeats = self.Params['Training']['Max Repeats Per Epoch']

                if (diff_ratio < ratio_limit and diff_ratio > 0):
                    break
                elif max_repeats > 0 and iter >= max_repeats:
                    print("AcceptEpoch = {} -- Loss = {:.4e} after {} repeats".format(
                        epoch, total_diff, iter))
                    break
                else:
            ...
            if (epoch % self.Params['Training']['Save Every * Epoch'] == 0) or (epoch == self.Params['Training']['Number of Epochs']) or (epoch == 1):
                self.plot(epoch,total_diff.item())
                with torch.no_grad():
    ...
    def Gradient(self, Xp):
        ...
        # #Xp.requires_grad_()
        # feature0, feature1 = self.network.env_features(Xp)
    ...
    def plot(self,epoch,total_train_loss):
        limit = 0.5
        xmin     = [-limit,-limit]
        xmax     = [limit,limit]
        spacing=limit/40.0

...
This is a **re-run of env1 only**, with settings identical to `experiments_lazyprm.md`
...
`experiments_lazyprm.md`.  Results land in `results/ompl_lazyprm_rerun/` so the
...
Sweep driver: `baselines/lazyprm_logs/run_sweep_env1_rerun.sh` (the 6 env1 configs in
parallel, one process each, `OMP_NUM_THREADS=1`).  Per-config logs land in
`baselines/lazyprm_logs/rerun_env1/<name>.log` and the full summaries in
...
1. **The env1 numbers in `experiments_lazyprm.md` understate LazyPRM badly** -- by
...
`experiments_rrt_connect.md` is far less exposed to this: it recorded one timeout in
